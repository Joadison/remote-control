...
function RemoteControlRoku() {
  ...
  async function send(command) {
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND}/roku/${rokuIp}/keypress/${command}`
      );
  ...
  async function launch(appId) {
    ...
    try {
      await fetch(`${process.env.REACT_APP_BACKEND}/roku/launch/${appId}`);
  ...
  async function sendText() {
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND}/roku/input?text=${encodeURIComponent(
          textInput
        )}`
      );
  ...
  async function fetchApps() {
    try {
      const res = await fetch(`${process.env.REACT_APP_BACKEND}/roku/query/apps`);
      console.log(res);
